...
| R17 | Zoomed out, the peek fan scales with the scene and becomes unreadable (overlapping titles, file names out of their pills). | P1, P6 | Done: The peek is scene markup under the scene's camera in its own layer, so the scene's rules style it at every level by construction; only its row spacing keeps a floor (22 px), so far-out labels never overlap. |
