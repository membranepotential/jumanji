...
- A node's **children** are the nodes it links to. Which links count as
  ...
  - **links** — every outgoing link, so a node can appear more than once
    (a link back to the root is a child like any other);
...
| Leave | `Esc`, `t` | — | Close the graph, back to the page. `q` quits jumanji, as everywhere. |
...
| Switch view | `v` | — | links ⇄ tree; the status line names the view; the selection stays. |
...
| Handle `+n` / `−`, muted, inside the pill's right edge (beside the label when zoomed out) | Folded, with `n` children in this view / unfolded. No handle: nothing to unfold here. Visible and clickable at every zoom level. Muted, because accent belongs to the route. |
...
| Accent **outline** + dimmed rest (tree view) | The selected node's link targets. Tree view shows every node once, so it cannot show links as children; it highlights them instead. |
| Panel, bottom left | The selection's full title and path; the route as a breadcrumb (hidden when the route is one node). |
| Status line | `Graph: links` / `Graph: tree`. |
...
owner's feedback on 2026-09-18, and a code review on 2026-09-19 (R17–R20).
Rows without "Done" are open.
...
| R18 | Zoomed far out, handles disappear: folds are invisible and cannot be clicked. | P1, P5 | Done: Handles stay visible and clickable at every level (a small muted `+n` / `−` beside the far label). |
