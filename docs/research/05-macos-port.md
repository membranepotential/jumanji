...
| `loadHTMLString:baseURL:` grants no local file read | **refuted** on macOS (spike, 2026-09-14) | With a `file://` base, WKWebView loads document-relative *and* absolute `file://` images from any directory, on a default `WKWebViewConfiguration` and without wry setting any file-access preference; confirmed wry-free in Swift. The secondary sources describe iOS / App-Sandboxed apps. What is missing is a wry API for the base — `WebView::load_html` takes only the string — so the mac shell either calls `loadHTMLString:baseURL:` through `WebViewExtMacOS` (Linux-identical semantics) or uses a custom protocol, which then has to proxy *every* local reference: a non-file origin cannot load absolute `file://` resources. See `spikes/macos-wkwebview/README.md` |
