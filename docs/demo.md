...
```sh
cargo run -- docs/demo.md
```

Drive it like zathura: `j`/`k` scroll (try `10j`), `J`/`K` jump sections,
`gg`/`G` top/bottom, `+`/`-`/`=` zoom, `/` search, `Ctrl-r` dark mode, `q` quit.
...
in pure Rust — no browser, no JavaScript, no network.
...
![two boxes and a line](../demo/assets/diagram.png)
